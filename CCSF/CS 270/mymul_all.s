...
mymul:  
    ...
    sw		$ra,20($sp)
    sw 		$a0,24($sp)	# $a0 = left
    sw		$a1,28($sp)	# $a1 = right
    sw		$s0,16($sp)	# $s0 = retval
    
    # you must call the function checkregs 
    jal     	checkregs
    # finish mymul here
  
    lw		$a0,24($sp)	# restore left
    lw 		$a1,28($sp)	# restore right
    
    bne 	$a0,1,Lmultiply	# if(left != 1) goto Lmultiply
    move 	$s0,$a1		# retval = right
    move 	$v0,$s0		# retval gets returned
    b		Lend		# goto Lend
    
    
Lmultiply:
    subi 	$a0,$a0,1	# $a0--
    jal 	mymul		# mymul(left - 1, right)
    lw		$a0,24($sp)	# restore left
    lw 		$a1,28($sp)	# restore right
    add 	$s0,$a1,$v0	# retval = right + mymul(left - 1, right)
    move 	$v0,$s0		# move retval to $v0 to be returned
